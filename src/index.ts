    ...
    try {
        ...
        for (const row of staleResults.rows) {
            ...
            await bot.telegram.sendMessage(
                row.user_id,
                '⚠️ Не смогли раздеть. Кредит возвращен.',
                { 
        ...
        await ctx.replyWithPhoto(
            { source: './assets/welcome.jpg' },
            {
                caption: 'Добро пожаловать! 👋\n\n' +
                        ...
                        'Правила пользования📄\n\n' +
                        '**https://telegra.ph/Polzovatelskoe-Soglashenie-Photowombot-10-30**\n\n' +
                        'Примеры раздетых фоток 🍓🔞\n\n' +
                        '**https://telegra.ph/Vazhnaya-instrukciya-11-06**\n\n' +
...
bot.on(message('photo'), async (ctx) => {
    ...
    try {
        ...
        processingMsg = await ctx.reply('⏳ Начинаю раздевать...');
